        ...
        for log_line in log_id.edi_log_line_ids:
            ...
            if not order:
                partner = self.env['res.partner'].sudo().search(
                    [('x_edi_accounting_id', '=', log_line.accounting_id), ('type', '=', 'invoice')], limit=1)
                if partner and product:
                    ...
                    if not log_line.store_number:
                        shipping_partner = partner.child_ids.filtered(lambda sp: sp.type == 'delivery') or partner.parent_id and partner.parent_id.child_ids.filtered(lambda sp: sp.type == 'delivery')
